...
const Modal = {
  ...
  confirm(title, body) {
    return new Promise(res => {
      qs('#confirm-h,#confirm-title').textContent = title;
...
class Renderer {
  ...
  _wrap(n) {
    ...
    if (n > 1) {
      ...
      const b = el('button', { class:'ink-btn ghost', onclick:()=>{ if(this.S.story) this.S.db.save(this.S.story); Router.push('dashboard'); }});
    ...
    this.app.db.all().then(stories => {
      ...
      stories.forEach(s => {
        ...
        const item = el('div', { class:'codex-entry', role:'listitem', tabindex:'0',
          'aria-label':`${s.externalize?.name||'Untitled'} — Phase ${s.phase}` });
        item.innerHTML = `
          <div class="codex-phase-icon" aria-hidden="true">${p.icon}</div>
          <div class="codex-body">
            <div class="codex-name">${trunc(s.externalize?.name ? `The ${s.externalize.name} Story` : 'Untitled Story')}</div>
    ...
    w.innerHTML = `
      ...
      <div class="field-section">
        <div class="field-wrap">
          ...
          <div class="field-ink-wrap">
            <input type="text" id="ext-name" class="field-ink" maxlength="60"
              placeholder="Give it a name…" value="${s.name||''}" aria-required="true">
        ...
        <div class="field-wrap">
          ...
          <span class="field-lore">If <em id="pn1">${pn}</em> had a physical form — a size, colour, texture, sound, smell — describe it.</span>
          <div class="field-ink-wrap">
            <textarea id="ext-form" class="field-ink" placeholder="Describe its presence…">${s.personification||''}</textarea>
        ...
        <div class="field-wrap">
          ...
          <span class="field-lore">What does <em id="pn2">${pn}</em> want you to believe about yourself or the world?</span>
          <div class="field-ink-wrap">
            <textarea id="ext-intent" class="field-ink" placeholder="What does it whisper?…" aria-required="true">${s.intent||''}</textarea>
  ...
  _p2() {
    ...
    w.innerHTML = `
      <div class="chapter-header">
        ...
        <div class="chapter-lore">Now we trace how far <em>${pn}</em> has reached. Understanding its scope — and crucially its limits — reveals where your alternative story can take root and grow.</div>
        <div class="chapter-timing">⏱ 10–15 min</div>
      </div>
    ...
    C.DOMAINS.forEach(d => {
      ...
      card.innerHTML = `
        ...
        <textarea class="field-ink" id="dom-${d.id}" style="min-height:90px"
          placeholder="How does ${pn} show up here?…" aria-label="${d.name}">${s[d.id]||''}</textarea>`;
    ...
    w.innerHTML = `
      ...
      <div class="field-section">
        <div class="field-wrap">
          ...
          <span class="field-lore">When did <em>${pn}</em> first appear? What was happening in your life then?</span>
          <textarea id="dec-hist" class="field-ink" placeholder="Trace its origins…">${s.history||''}</textarea>
        </div>
        <div class="field-wrap">
          <label class="field-label" for="dec-cult">Cultural &amp; Social Support</label>
          <span class="field-lore">What messages from family, culture, or society does <em>${pn}</em> draw power from?</span>
          <textarea id="dec-cult" class="field-ink" placeholder="What wider narratives sustain it?…">${s.culturalSupport||''}</textarea>
        </div>
        <div class="field-wrap">
          ...
          <textarea id="dec-help" class="field-ink" placeholder="People, moments, or places that offered another view…">${s.whoHelped||''}</textarea>
        </div>
      </div>`;
  ...
  _p4() {
    ...
    w.innerHTML = `
      ...
      <div class="illumined-qs" aria-label="Guiding questions">
        <div class="illumined-q">💫 When did <em>${pn}</em> not totally win?</div>
        <div class="illumined-q">🔥 What would <em>${pn}</em> hate to know about you?</div>
        <div class="illumined-q">⚡ When did you surprise <em>${pn}</em> — even a little?</div>
      </div>
    ...
    const renderScrolls = () => {
      ...
      moments.forEach((m,i) => {
        ...
        card.innerHTML = `
          ...
          ${m.situation?`<div class="scroll-field"><div class="scroll-field-label">Situation</div><div class="scroll-field-value">${m.situation}</div></div>`:''}
          ${m.action?   `<div class="scroll-field"><div class="scroll-field-label">Your Action</div><div class="scroll-field-value">${m.action}</div></div>`:''}
          ${m.reveals?  `<div class="scroll-field"><div class="scroll-field-label">What It Reveals</div><div class="scroll-field-value">${m.reveals}</div></div>`:''}
          ${m.witness?  `<div class="scroll-field"><div class="scroll-field-label">Witness</div><div class="scroll-field-value">${m.witness}</div></div>`:''}
  ...
  _p5() {
      ...
      <div class="field-section">
        <div class="field-wrap">
          ...
          <input type="text" id="ra-title" class="field-ink" maxlength="80"
            placeholder="e.g. The Resilient Wanderer, The Tender Warrior…" value="${s.newName||''}">
        </div>
        ...
        <div class="field-wrap">
          ...
          <div class="field-ink-wrap">
            <textarea id="ra-story" class="field-ink" style="min-height:200px"
              placeholder="Begin: &quot;Despite ${pn}'s attempts, I have shown that I am someone who…&quot;"
              aria-required="true">${s.alternativeStory||''}</textarea>
        ...
        <div class="field-wrap">
          ...
          <textarea id="ra-vals" class="field-ink" style="min-height:80px"
            placeholder="e.g. connection, courage, honesty, creativity, tenderness…">${s.values||''}</textarea>
        </div>
    ...
    const renderClaims = () => {
      ...
      (this.S.story?.reauthor?.identityClaims||[]).forEach((c,i)=>{
        ...
        row.innerHTML = `<div class="claim-numeral" aria-hidden="true">${i+1}</div><div class="claim-text">${c}</div><button class="claim-erase" aria-label="Remove claim ${i+1}">✕</button>`;
    ...
    w.innerHTML = `
      ...
      <div class="field-section">
        <div class="field-wrap">
          ...
          <div class="field-ink-wrap">
            <textarea id="wit-decl" class="field-ink" style="min-height:150px"
              ...
              aria-required="true">${s.declaration||''}</textarea>
        ...
        <div class="field-wrap">
          ...
          <textarea id="wit-next" class="field-ink" placeholder="What will you do to embody your new story?…">${s.nextSteps||''}</textarea>
        </div>
        ...
        <div class="field-wrap">
          ...
          <textarea id="wit-commit" class="field-ink" placeholder="I commit to…">${s.commitments||''}</textarea>
        </div>
      </div>`;
    ...
    const renderGuild = () => {
      ...
      team.forEach(p => {
        ...
        row.innerHTML = `
          ...
          <div class="guild-info"><div class="guild-name">${p.name}</div>${p.role?`<div class="guild-role">${p.role}</div>`:''}</div>
          <button class="guild-dismiss" data-id="${p.id}" aria-label="Remove ${p.name}">✕</button>`;
  ...
  _summary() {
    ...
    page.innerHTML = `
      <div class="completion-crown">
        <h1 class="completion-title">${r.newName || 'Your Re-authored Story'}</h1>
      ...
      <div class="summary-chapter">
        ...
        <div class="summary-entry"><div class="summary-entry-label">Name</div><div class="summary-entry-value">${ex.name}</div></div>
        ${ex.intent?`<div class="summary-entry"><div class="summary-entry-label">Its Intent</div><div class="summary-entry-value">${ex.intent}</div></div>`:''}
      ...
      <div class="summary-chapter">
        ...
        ${sp.moments.map((m,i)=>`
          <div class="summary-entry">
            ...
            <div class="summary-entry-value">${m.situation}${m.reveals?`<br><em style="color:var(--text-3)">${m.reveals}</em>`:''}</div>
          </div>`).join('')}
      </div>`:''}
      ...
      <div class="summary-chapter">
        ...
        <div class="declaration-folio">${r.alternativeStory}</div>
      </div>`:''}
      ...
      <div class="summary-chapter">
        ...
        <div class="declaration-folio">${w.declaration}</div>
      </div>`:''}
